...
public class DojoCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§6§lDOJO §7» §7Usage: /dojo <duel|accept|deny|rank|arena>");
            return true;
        }
    ...
    private boolean handleGuardianSpawn(CommandSender sender, String[] args) {
        if (!sender.hasPermission("dojo.admin")) {
            sender.sendMessage("§6§lDOJO §7» §cYou do not have permission to spawn guardians.");
            return true;
        }
        if (!(sender instanceof Player player)) {
            sender.sendMessage("§cOnly players can use this command.");
        ...
        DojoGuardian.spawn(player.getLocation(), tier);
        player.sendMessage("§6§lDOJO §7» §aSpawned a tier " + tier + " Dojo Guardian.");
        return true;
    }
}
